%% @author WangLiangyou <leonwang.beyond@gmail.com>
%% @copyright 2015 WangLiangyou <leonwang.beyond@gmail.com>
%% 
...
run_pathfinder_test(WorldDiagram, OptimalPathLength, SearchOptions) ->
	{_, WorldCoords} =
		lists:foldl(fun(Row, {RowN,RowData}) ->
							{_, ColData} =
								lists:foldl(fun(Col, {ColN,RowColData}) ->
	...
	{Start, End = {EndX,EndY}} =
		lists:foldl(fun
					   ({Coord, Char}, {undefined,AccEnd}) when Char == StartChar ->
							{Coord, AccEnd};
					   ({Coord, Char}, {AccStart,undefined}) when Char == EndChar ->
							{AccStart, Coord};
					   ({_, _}, Acc) ->
							Acc
					end, {undefined, undefined}, WorldCoords),
	MaxX = length(lists:nth(1, WorldDiagram)),
	MaxY = length(WorldDiagram),
	NeighbourFn =
		fun ({X,Y}) ->
				 Neighbours =
					 lists:filter(fun({_Dir, Coord}) ->
	...
	ScoreFn =
		fun(Path, {X,Y}) ->
				if {X,Y} == End ->
					   max;
				   true ->
					   ValueTmp = -length(Path) - math:sqrt(abs(X-EndX)*abs(X-EndX)+abs(Y-EndY)*abs(Y-EndY)),
					   ValueTmp
				end
		end,
	case astar:search(Start, NeighbourFn, ScoreFn, SearchOptions) of
		{{worklimited, _BestScore}, _BestPathRev, _BestState} ->
		...
		{BestScore, BestPathRev, _BestState}
		  when (BestScore == max) or (is_integer(BestScore)) ->
			...
			lists:foldl(fun({XC,YC}, {X,Y}) ->
								NewPos = {X+XC,Y+YC},
								if NewPos == End -> true;
								   true -> ets:insert(World, {NewPos, $o}) end,
								NewPos
						end, Start, BestPath),
			BestPathDiagram =
				lists:foldl(fun(Y, YA) ->
									[lists:foldl(fun(X, XA) ->
														 [{{X,Y}, Contents}] = ets:lookup(World, {X,Y}),
														 [Contents | XA]
												 end, "\n", lists:reverse(lists:seq(1, MaxX))) | YA]
							end, [], lists:reverse(lists:seq(1, MaxY))),
